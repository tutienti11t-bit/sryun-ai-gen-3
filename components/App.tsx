...
const App: React.FC = () => {
  ...
  const [adminOmniscience, setAdminOmniscience] = useState(false);
  ...
  const unlockAllUnits = () => {
    setUser(prev => {
      localStorage.setItem('sryun_unlocked_units', '12');
      return { ...prev, unlockedUnits: 12 };
    });
    document.body.classList.add('animate-shake-once');
    setTimeout(() => document.body.classList.remove('animate-shake-once'), 500);
  };
  ...
  const renderContent = () => {
    switch (view) {
      ...
      case 'units':
        return <UnitSelection unlockedUnits={user.unlockedUnits} unitScores={unitScores} onSelect={(unit) => { setSelectedUnitCat(unit); setView('unit-hub'); }} />;
      case 'unit-hub':
        return <UnitHub unitName={selectedUnitCat} onSelectModule={(v) => setView(v)} onBack={() => setView('units')} />;
      case 'vocab': 
        return <VocabTheme unlockedUnits={user.unlockedUnits} selectedUnit={selectedUnitCat} setSelectedUnit={setSelectedUnitCat} onNext={() => setView('unit-hub')} />;
      case 'grammar': 
        return <GrammarView unit={parseInt(selectedUnitCat.match(/\d+/)?.[0] || "1")} onNext={() => setView('unit-hub')} onBack={() => setView('unit-hub')} />;
      case 'unit-test': 
        return <GatewayTest unit={parseInt(selectedUnitCat.match(/\d+/)?.[0] || "1")} onSuccess={handleUnitUnlock} onBack={() => setView('unit-hub')} adminShowAnswer={adminOmniscience} />;
      case 'listening': 
        return (
          <div className="animate-fadeIn">
            ...
            <ListeningView initialTopic={selectedUnitCat} onComplete={handleXPChange} onNavigate={setView} adminShowAnswer={adminOmniscience} />
          </div>
        );
      case 'reading': 
        return (
      ...
      case 'writing': 
        return (
          <div className="animate-fadeIn">
            <button onClick={() => setView('unit-hub')} className="mb-8 px-6 py-3 glass rounded-full text-[10px] font-black uppercase text-white hover:bg-white/10 transition-all">← Quay lại Unit</button>
            <WritingView initialTopic={selectedUnitCat} onComplete={handleXPChange} onNavigate={setView} />
          </div>
        );
      case 'homework':
        return <HomeworkView unitName={selectedUnitCat} onComplete={handleXPChange} adminShowAnswer={adminOmniscience} />;
      case 'homework':
        return <HomeworkView unitName={selectedUnitCat} onComplete={handleXPChange} adminShowAnswer={adminOmniscience} />;
      case 'lesson-plan':
      ...
      case 'games':
        ...
        return (
          <div className="animate-fadeIn">
            ...
            {gameType === 'quiz' && <QuizGame onComplete={handleXPChange} adminShowAnswer={adminOmniscience} />}
            {gameType === 'crossword' && <CrosswordGame onComplete={handleXPChange} adminShowAnswer={adminOmniscience} />}
            {gameType === 'builder' && <SentenceBuilder onComplete={handleXPChange} adminShowAnswer={adminOmniscience} />}
            {gameType === 'listening' && <ListeningHero onComplete={handleXPChange} adminShowAnswer={adminOmniscience} />}
          </div>
        );
      case 'profile':
        return (
            ...
            <div className="bento-grid">
              ...
              <div className="md:col-span-2 glass-card p-8 rounded-[3rem] border border-white/10 space-y-6">
                ...
                <div className="space-y-4">
                  {[
                    { label: 'Vocabulary', val: 75, color: 'bg-indigo-500' },
                    { label: 'Grammar', val: 60, color: 'bg-purple-500' },
                    { label: 'Listening', val: 45, color: 'bg-emerald-500' },
                    { label: 'Writing', val: 30, color: 'bg-rose-500' }
                  ].map(s => (
                    <div key={s.label}>
  ...
  return (
    <div className="min-h-screen flex selection:bg-indigo-500 selection:text-white">
      ...
      <div className={`fixed bottom-24 right-6 z-[60] flex flex-col items-end gap-3 transition-all duration-500 ${showAdminPanel ? 'opacity-100 translate-y-0' : 'opacity-40 hover:opacity-100 translate-y-10'}`}>
        {showAdminPanel && (
          <div className="glass p-6 rounded-[2.5rem] border border-rose-500/30 shadow-[0_0_50px_rgba(244,63,94,0.2)] space-y-5 animate-fadeIn min-w-[240px]">
            <div className="flex items-center gap-3 border-b border-white/10 pb-3 mb-1">
              <span className="text-xl animate-pulse">☢️</span>
              <span className="text-[10px] font-black text-rose-400 uppercase tracking-widest">God Mode Console</span>
            </div>
            
            <div className="space-y-3">
              <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Ultimate Power ⚡</p>
              <button 
                onClick={unlockAllUnits} 
                className="w-full py-3 bg-rose-600 hover:bg-rose-500 text-white rounded-xl text-[10px] font-black border border-rose-400 shadow-lg transition-all active:scale-95"
              >
                🔓 UNLEASH ALL UNITS
              </button>
              
              <button 
                onClick={() => setAdminOmniscience(!adminOmniscience)} 
                className={`w-full py-3 rounded-xl text-[10px] font-black border transition-all flex items-center justify-center gap-2 ${
                  adminOmniscience ? 'bg-emerald-600 border-emerald-400 text-white' : 'bg-white/5 border-white/20 text-slate-400 hover:bg-white/10'
                }`}
              >
                {adminOmniscience ? '👁️ OMNISCIENCE ON' : '👁️ SHOW ANSWERS'}
              </button>
            </div>

            <div className="space-y-2">
              <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Level Mastery 💎</p>
              <div className="flex gap-2">
                <button onClick={() => handleXPChange(1000)} className="flex-1 py-2 bg-indigo-500/20 text-indigo-400 rounded-xl text-[10px] font-black border border-indigo-500/30 hover:bg-indigo-500 hover:text-white transition-all">+1K</button>
                <button onClick={() => handleXPChange(20000)} className="flex-1 py-2 bg-purple-500/20 text-purple-400 rounded-xl text-[10px] font-black border border-purple-500/30 hover:bg-purple-500 hover:text-white transition-all">MAX</button>
              </div>
            </div>
            
            <div className="space-y-2">
              <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Time Bender 🔥</p>
              <div className="flex gap-2">
                <button onClick={() => handleStreakChange(1)} className="flex-1 py-2 bg-orange-500/20 text-orange-400 rounded-xl text-[10px] font-black border border-orange-500/30 hover:bg-orange-500 hover:text-white transition-all">+1</button>
                <button onClick={() => handleStreakChange(365)} className="flex-1 py-2 bg-orange-500/20 text-orange-400 rounded-xl text-[10px] font-black border border-orange-500/30 hover:bg-orange-500 hover:text-white transition-all">365</button>
              </div>
            </div>
          </div>
        )}
        ...
        <button 
          ...
          className={`w-14 h-14 rounded-full flex items-center justify-center text-xl shadow-2xl transition-all border border-white/20 ${showAdminPanel ? 'bg-rose-500 text-white rotate-45' : 'glass text-indigo-400'}`}
        >
          {showAdminPanel ? '×' : '⚙️'}
        </button>
      </div>

      <Sidebar currentView={view} setView={(v) => { setView(v); setGameType('selection'); setIsEditing(false); }} />
      
      <main className="flex-1 md:ml-80 p-8 md:p-12 transition-all">
      ...
      <div className="md:hidden fixed bottom-6 left-6 right-6 z-50">
        <div className="glass p-4 rounded-[2.5rem] flex justify-around items-center border border-white/10 shadow-2xl">
          {[{ id: 'home', icon: '🏠' }, { id: 'units', icon: '🎯' }, { id: 'ai-tutor', icon: '🤖' }, { id: 'profile', icon: '👤' }].map(item => (
            <button 
              key={item.id} 
              onClick={() => setView(item.id as AppView)} 
              className={`text-2xl w-14 h-14 flex items-center justify-center rounded-2xl transition-all ${view === item.id ? 'bg-indigo-600 text-white shadow-xl scale-110' : 'text-slate-500'}`}
            >
              {item.icon}
            </button>
          ))}
